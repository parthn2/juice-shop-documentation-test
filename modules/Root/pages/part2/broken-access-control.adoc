...
link:injection.md#log-in-with-the-administrators-user-account[logged in with the administrator account]
...
link:cryptographic-issues.md#apply-some-advanced-cryptanalysis-to-find-the-real-easter-egg[a follow-up challenge]
to this one._
...
you used in
link:injection.md#infect-the-server-with-juicy-malware-by-abusing-arbitrary-command-execution[Infect the server with juicy malware by abusing arbitrary command execution].
